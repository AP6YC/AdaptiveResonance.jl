...
mutable struct XB <: AbstractCVI
    ...
    n::Array{Int64, 1}          # n_clusters
    v::Array{Float64, 2}        # dim x n_clusters
    CP::Array{Float64, 1}       # n_clusters
    SEP::Float64
...
function param_inc!(cvi::XB, sample::Array{T, 1}, label::I) where {T<:Real, I<:Int}
    ...
    if label > cvi.n_clusters
        ...
        # push!(cvi.CP, CP_new)
        # push!(cvi.n, n_new)
        cvi.CP = expand_array(cvi.CP, n=(label-cvi.n_clusters))
        cvi.CP[label] = CP_new
        cvi.n = expand_array(cvi.n, n=(label-cvi.n_clusters))
        cvi.n[label] = n_new
        # Update 2-D parameters with appending and reassignment
        # cvi.v = [cvi.v v_new]
        cvi_v_new = zeros(cvi.dim, label)
        cvi_v_new[:, label] = v_new
        cvi.v = cvi_v_new
        # cvi.G = [cvi.G G_new]
        cvi_G_new = zeros(cvi.dim, label)
        cvi_G_new[:, label] = G_new
        cvi.G = cvi_G_new
